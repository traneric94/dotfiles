function get_profile() {
  if [ "$1" == "prod" ]; then profile="prod"; else profile="nonprod"; fi
  echo "$profile"
}

function find_secure_ip() {
    aws ec2 describe-instances --profile $(get_profile "$1") --region us-west-2 --filters "Name=tag-value,Values=${1}-secure" --query 'Reservations[*].Instances[*].NetworkInterfaces[*].PrivateIpAddress' --output text | head -n1
}
...
function tunnel() {
    ...
    ssh -L ${port}:$(find_secure_ip "$1"):22 -f -N $(find_bastion_ip "$1")
